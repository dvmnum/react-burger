...
type FODProps = {
    fetchedData?: IFeedOrder
}

export const FeedOrderUserDetails: React.FC<FODProps> = (fetchedData) => {
    const { feed } = useAppSelector(store => store.wsFeedUserReducer)
    const ingredients = useAppSelector(store => store.ingredientsReducer.ingredients)

    let feed_number = useParams().number

    const data = !fetchedData.fetchedData && feed !== null ? feed.orders && feed.orders.filter((item: IFeedOrder) => item.number.toString() === feed_number)[0] : fetchedData.fetchedData

    const itemPrice = useMemo(() => (
        (data as IFeedOrder).ingredients
        ...
        {data &&
            <>
                ...
                {data.ingredients
                    ...
                    <li key={index} className={styles.feed_li}>
